...
impl TeleText {
    ...
    fn skip_next_char(&mut self, chr: char) -> ParserResult<()> {
        let chr_start = self.current_text.find(chr).ok_or(ParseErr::InvalidPage)?;
    ...
    fn skip_next_string(&mut self, string: &str) -> ParserResult<()> {
        let string_start = self
            .current_text
            .find(string)
            .ok_or(ParseErr::InvalidPage)?;
    ...
    fn skip_next_tag(&mut self, tag: &str, closing: bool) -> ParserResult<()> {
    ...
    fn parse_middle_link(&mut self) -> ParserResult<Result<HtmlLink, HtmlText>> {
        let link = self.parse_current_link()?;
        if link.url.len() != HTML_LINK_SIZE {
            return Ok(Err(link.inner_text));
        }

        Ok(Ok(link))
    }

    fn parse_middle(&mut self) -> ParserResult<()> {
        ...
        while !self.current_text.starts_with("</pre>") {
            ...
            let line_len = self.current_text.find('\r').ok_or(ParseErr::InvalidPage)?;
    ...
    fn parse_sub_pages(&mut self) -> ParserResult<()> {
        ...
        while !self.current_text.starts_with("</p>") {
            match self.get_tag_type() {
                ...
                _ => {
                    let link_start = self.current_text.find('<').ok_or(ParseErr::InvalidPage)?;
    ...
    fn parse_bottom_navigation(&mut self) -> ParserResult<()> {
        self.skip_next_tag("p", false)?;
    ...
    pub fn parse(mut self, loader: HtmlLoader) -> ParserResult<Self> {
